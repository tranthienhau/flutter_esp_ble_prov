 ...
 */
object EspBleProvErrorCodes {
    const val BLE_SCAN_FAILED = "BLE_SCAN_FAILED"
    const val DEVICE_NOT_FOUND = "DEVICE_NOT_FOUND"
    const val CONNECTION_FAILED = "CONNECTION_FAILED"
    const val AUTH_FAILED = "AUTH_FAILED"
    const val WIFI_SCAN_FAILED = "WIFI_SCAN_FAILED"
  ...
  override fun call(ctx: CallContext) {
    ...
    boss.espManager.searchBleEspDevices(prefix, object : BleScanListener {
      ...
      override fun onFailure(e: java.lang.Exception?) {
        boss.e("BleScanManager: onFailure $e")
        // Use the native exception message directly as it's already descriptive
        ctx.result.error(EspBleProvErrorCodes.BLE_SCAN_FAILED, e?.message ?: "BLE scan failed", null)
      }
    ...
    boss.connect(conn, proofOfPossession) { esp ->
      ...
      esp.scanNetworks(object : WiFiScanListener {
        ...
        override fun onWiFiScanFailed(e: java.lang.Exception?) {
          ...
          // Use the native exception message directly as it's already descriptive
          ctx.result.error(EspBleProvErrorCodes.WIFI_SCAN_FAILED, e?.message ?: "WiFi scan failed", null)
        }
      })
    }
  }
    ...
    boss.connect(conn, proofOfPossession) { esp ->
      ...
      esp.provision(ssid, passphrase, object : ProvisionListener {
        override fun createSessionFailed(e: java.lang.Exception?) {
          ...
          // Use the native exception message directly
          ctx.result.error(EspBleProvErrorCodes.CONNECTION_FAILED, e?.message ?: "Failed to create session", null)
        }
        ...
        override fun wifiConfigFailed(e: java.lang.Exception?) {
          ...
          // Use the native exception message directly
          ctx.result.error(EspBleProvErrorCodes.PROVISION_FAILED, e?.message ?: "WiFi configuration failed", null)
        }
        ...
        override fun wifiConfigApplyFailed(e: java.lang.Exception?) {
          ...
          // Use the native exception message directly
          ctx.result.error(EspBleProvErrorCodes.PROVISION_FAILED, e?.message ?: "Failed to apply WiFi configuration", null)
        }

        override fun provisioningFailedFromDevice(failureReason: ESPConstants.ProvisionFailureReason?) {
          boss.e("provisioningFailedFromDevice $failureReason")
          // Provide descriptive message based on failure reason
          val message = when (failureReason) {
            ESPConstants.ProvisionFailureReason.AUTH_FAILED -> "Authentication failed. Please check your WiFi password."
            ESPConstants.ProvisionFailureReason.NETWORK_NOT_FOUND -> "WiFi network not found. Please check the SSID."
            ESPConstants.ProvisionFailureReason.UNKNOWN -> "Provisioning failed with unknown reason"
            else -> "Provisioning failed: ${failureReason?.name ?: "Unknown reason"}"
          }
          ctx.result.error(EspBleProvErrorCodes.PROVISION_FAILED, message, null)
        }
        ...
        override fun onProvisioningFailed(e: java.lang.Exception?) {
          ...
          // Use the native exception message directly
          ctx.result.error(EspBleProvErrorCodes.PROVISION_FAILED, e?.message ?: "Provisioning failed", null)
        }
